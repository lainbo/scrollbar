  ...
  <div class="mainContainer">
    <div class="settingsContainer">
      ...
      <div class="githubBtnContainer">
        <a
          :class="theme"
          href="https://github.com/lainbo/scrollbar"
          class="githubBtn"
        >
          <img
            v-if="theme === 'light'"
      ...
      <div class="firefoxMsgContainer" v-if="isFirefox">
        <span class="alert">
          ...
          <span class="alertMsg">Firefox 不支持！</span>
        </span>
        <p class="isFirefoxMsg">看起来您正在使用 Firefox。</p>
        <p class="isFirefoxMsg">
          建议使用基于 WebKit 或 Blink 的浏览器来获得最佳体验，例如 Chrome、Safari、Edge、Opera、Brave 等。
        </p>
        <p class="isFirefoxMsg">
          Firefox 不支持
          <span class="highlight">::-webkit-scrollbar</span> 选择器。
        </p>
        <p class="isFirefoxMsg">
          要在 Firefox 上自定义滚动条，您应该使用
          <span class="highlight">scrollbar-color</span> 和
          <span class="highlight">scrollbar-width</span> 选择器。
        </p>
      </div>
    </div>
    <div class="codeOutputContainer">
      <h2 :class="theme">代码</h2>
...
@supports not selector(::-webkit-scrollbar) {
  body {
      ...
      <button @click="copyCSSCode" class="btn">
        <span v-if="!isCSSTextCopied">复制代码</span
        ><span v-if="isCSSTextCopied">已复制！</span>
      </button>
    </div>
  </div>
</template>
